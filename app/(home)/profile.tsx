...
export default function ProfileScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      supabase
        .from('profiles')
        .select('is_pro_version, subscription_plan, credits_current, family_role, email, display_name')
        .eq('id', user.id)
        .single()
        .then(({ data }) => {
          if (data) {
            ...
            const savedName = data.display_name && !data.display_name.startsWith('anon_')
              ? data.display_name
              : null;
            if (savedName) {
              setProfileDisplayName(savedName);
            } else {
              // No real name saved yet — prompt once
              const metaName = user.user_metadata?.display_name ?? '';
              const isAutoGenerated = !metaName || metaName.startsWith('anon_') || metaName === aEmail.split('@')[0];
              if (isAutoGenerated) {
                setEditName('');
                setEditEmail(aEmail.startsWith('anon_') ? '' : aEmail);
                setEditRole(null);
                setPromptError(null);
      ...
      supabase
        ...
        .then(({ count }) => setGenerationsCount(count ?? 0));
  ...
  const isFakeAnonEmail = authEmail.startsWith('anon_');
  const displayName = profileDisplayName
    ?? (isFakeAnonEmail
      ? (user?.user_metadata?.display_name ?? 'Guest')
      : (user?.user_metadata?.display_name ?? authEmail.split('@')[0] ?? 'User'));
  const email = profileEmail || (isFakeAnonEmail ? '' : authEmail);
  ...
  async function handleSaveProfile() {
    ...
    if (isNamePrompt) {
      if (!newName) { setPromptError('Please enter your name.'); return; }
      if (!editRole) { setPromptError('Please select your family role.'); return; }
    }
    setPromptError(null);
    ...
    try {
      ...
      const emailChanged = !isNamePrompt && !!newEmail && newEmail !== email;
      const roleChanged = isNamePrompt && !!editRole && editRole !== familyRole;
      ...
      if (nameChanged) profileUpdates.display_name = newName;
      if (roleChanged) profileUpdates.family_role = editRole!;
      ...
      if (nameChanged || roleChanged) {
        ...
        if (nameChanged) setProfileDisplayName(newName);
        if (roleChanged) setFamilyRole(editRole);
      }
  ...
  async function handleRestorePurchases() {
    if (iapService.isSimulationMode()) {
      Alert.alert('Not Available', 'Restore purchases requires a production build.');
      return;
    }
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        {/* EDIT PROFILE MODAL */}
        <Modal visible={editOpen} transparent animationType="slide" onRequestClose={() => { if (!isNamePrompt) setEditOpen(false); }}>
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <Pressable style={s.roleModalOverlay} onPress={() => { if (!isNamePrompt) setEditOpen(false); }}>
              <Pressable style={s.roleModalSheet} onPress={() => {}}>
                <View style={s.roleModalHandle} />
                <Text style={s.roleModalTitle}>{isNamePrompt ? 'Set Up Your Profile' : 'Edit Profile'}</Text>

                {!isNamePrompt && (
                  <>
                    <Text style={s.inputLabel}>Profile Picture</Text>
                    <View style={s.avatarPickerRow}>
                      <View style={s.avatarPickerPreview}>
                        {uploadingAvatar ? (
                          <ActivityIndicator color='#fff' />
                        ) : avatarUrl ? (
                          <Image source={{ uri: avatarUrl }} style={s.avatarPickerImg} />
                        ) : (
                          <Text style={s.avatarText}>{initials}</Text>
                        )}
                      </View>
                      <TouchableOpacity style={s.avatarPickerBtn} onPress={handlePickAvatar} disabled={uploadingAvatar} activeOpacity={0.8}>
                        <MaterialIcons name='photo-camera' size={16} color={C.primary} />
                        <Text style={s.avatarPickerBtnText}>{avatarUrl ? 'Change Photo' : 'Add Photo'}</Text>
                      </TouchableOpacity>
                    </View>
                  </>
                )}
                ...
                <TextInput
                  style={s.editInput}
                  value={editName}
                  onChangeText={(v) => { setEditName(v); setPromptError(null); }}
                ...
                {isNamePrompt && (
                  <>
                    <Text style={s.inputLabel}>Family Role</Text>
                    <View style={s.roleChipsWrap}>
                      {['Mom', 'Dad', 'Grandma', 'Grandpa', 'Sister', 'Brother', 'Aunt', 'Uncle', 'Other'].map((role) => (
                        <TouchableOpacity
                          key={role}
                          style={[s.roleChip, editRole === role && s.roleChipActive]}
                          onPress={() => { setEditRole(role); setPromptError(null); }}
                          activeOpacity={0.7}
                        >
                          <Text style={[s.roleChipText, editRole === role && s.roleChipTextActive]}>{role}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  </>
                ...
                {promptError && <Text style={s.promptErrorText}>{promptError}</Text>}
                ...
                >
                  {saving ? (
                    ...
                    <Text style={s.familyPrimaryBtnText}>{isNamePrompt ? 'Continue' : 'Save Changes'}</Text>
                  )}
                </TouchableOpacity>
              </Pressable>
            </Pressable>
        ...
        <View style={s.card}>
          ...
          <SettingsRow
            icon="star-outline"
            title="Subscription"
            subtitle="View or manage your plan"
...
const s = StyleSheet.create({
  ...
  avatarImage: { width: 80, height: 80, borderRadius: 999 },
  avatarText: { fontSize: 28, fontWeight: '800', color: C.primary },

  avatarPickerRow: { flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 16 },
  avatarPickerPreview: {
    width: 56, height: 56, borderRadius: 999,
    backgroundColor: C.primaryLight, alignItems: 'center', justifyContent: 'center',
    overflow: 'hidden',
  },
  avatarPickerImg: { width: 56, height: 56, borderRadius: 999 },
  avatarPickerBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8,
    borderWidth: 1, borderColor: C.border, borderRadius: 10,
    paddingVertical: 10, paddingHorizontal: 14, backgroundColor: C.surface,
  },
  avatarPickerBtnText: { fontSize: 14, color: C.primary, fontWeight: '600' },
  ...
  roleChipsWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 },
  roleChip: {
    paddingVertical: 8, paddingHorizontal: 14, borderRadius: 999,
    borderWidth: 1.5, borderColor: C.border, backgroundColor: C.surface,
  },
  roleChipActive: { borderColor: C.primary, backgroundColor: '#EEF2E6' },
  roleChipText: { fontSize: 14, fontWeight: '600', color: C.textSecondary },
  roleChipTextActive: { color: C.primary },

  promptErrorText: { fontSize: 13, color: C.error, marginBottom: 10, textAlign: 'center' },
});
